...
import { binaryRushPath, hexBlitzPath, packetDefenderPath, portHunterPath } from "@/lib/paths";
import { hexText } from "./hex-blitz/content";
...
export default function GamesIndex({ lang }: { lang: Locale }) {
  ...
  return <main id="main" className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
    ...
    <section className="mt-10 grid gap-5 md:grid-cols-2" aria-label={t.indexTitle}>
      ...
      <article className="rounded-2xl border border-violet-300 bg-gradient-to-br from-violet-50 to-fuchsia-50 p-6 shadow-sm dark:border-violet-900 dark:from-slate-950 dark:to-violet-950 md:col-span-2"><div className="flex items-center justify-between"><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">✓ {t.available}</span><span className="text-sm font-semibold">{t.free}</span></div><h2 className="mt-5 font-mono text-2xl font-bold">Hex Blitz</h2><p className="mt-2 text-gray-600 dark:text-neutral-300">{hex.desc}</p><Link href={hexBlitzPath(lang)} className="mt-6 inline-flex min-h-11 items-center rounded-lg bg-violet-700 px-5 py-3 font-bold text-white hover:bg-violet-800">{t.play} →</Link></article>
      <div className="grid grid-cols-2 gap-3 md:col-span-2">{["Subnet Sprint","OSI Stack"].map(name=><article key={name} className="rounded-xl border border-dashed bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900"><span className="text-xs font-semibold uppercase text-gray-500">{t.soon}</span><h2 className="mt-2 font-mono font-bold">{name}</h2></article>)}</div>
    </section>
  </main>;
}
