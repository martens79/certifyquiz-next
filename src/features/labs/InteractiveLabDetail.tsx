...
export default function InteractiveLabDetail({lang,lab}:{lang:Locale;lab:LabSlug}){const href=lang==="en"?"/interactive-labs":`/${lang}/interactive-labs`;const complete=()=>{};return <main id="main" className="min-h-[75vh] bg-slate-50 px-3 py-8 sm:px-4 sm:py-12"><div className="mx-auto max-w-6xl"><Link href={href} className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-indigo-700 hover:text-indigo-900"><ArrowLeft size={17}/>{back[lang]}</Link>{lab==="spreadsheets"?<SpreadsheetFormulaLab lang={lang} onComplete={complete}/>:lab==="networking"?<NetworkTroubleshootingLab lang={lang} onComplete={complete}/>:lab==="phishing"?<PhishingAnalysisLab lang={lang} onComplete={complete}/>:scenarioChallenges[lab]?<ScenarioChallengeLab lang={lang} slug={lab} onComplete={complete}/>:null}</div></main>}
