...
export default function GuidedCertificationLab({lang,slug}:{lang:Locale;slug:string}) {
  ...
  if(result)return <section className="rounded-2xl border bg-white p-6 text-center shadow-sm sm:p-10"><CheckCircle2 className={`mx-auto h-14 w-14 ${result.passed?"text-emerald-600":"text-amber-600"}`}/><p className="mt-4 text-sm font-bold uppercase tracking-wider text-slate-500">{t.result}</p><h2 className="mt-2 text-3xl font-black">{result.score}% — {result.passed?t.passed:t.failed}</h2>{result.solution?.explanation&&<p className="mx-auto mt-4 max-w-2xl leading-7 text-slate-600">{result.solution.explanation}</p>}<button onClick={restart} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-indigo-700 px-5 py-3 font-bold text-white"><RotateCcw size={17}/>{t.restart}</button></section>;

  return <article className="overflow-hidden rounded-2xl border bg-white shadow-xl"><header className="bg-gradient-to-r from-slate-950 to-indigo-950 p-5 text-white sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><span className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-cyan-300"><ShieldCheck size={16}/>{CERTIFICATION_LABELS[lab.certificationSlug] ?? lab.certificationSlug}</span><span className="flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-sm"><Clock3 size={15}/>{lab.estimatedMinutes} min</span></div><h1 className="mt-4 text-2xl font-black sm:text-4xl">{lab.title}</h1><p className="mt-3 max-w-3xl text-slate-200">{lab.description}</p><div className="mt-6 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full bg-cyan-400 transition-all" style={{width:`${((index+(feedback==="correct"?1:0))/lab.content.steps.length)*100}%`}}/></div></header><div className="grid lg:grid-cols-[300px_1fr]"><aside className="border-b bg-slate-50 p-5 lg:border-b-0 lg:border-r sm:p-6"><h2 className="text-xs font-black uppercase tracking-wider text-slate-500">{t.scenario}</h2><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-700">{lab.content.scenario}</p><h2 className="mt-6 text-xs font-black uppercase tracking-wider text-slate-500">{t.objective}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{lab.content.objective}</p><h2 className="mt-6 text-xs font-black uppercase tracking-wider text-slate-500">{t.prerequisites}</h2><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700">{lab.content.prerequisites.map(x=><li key={x}>{x}</li>)}</ul></aside><section className="min-w-0 p-5 sm:p-8"><p className="text-sm font-bold text-indigo-700">{index+1} / {lab.content.steps.length}</p><h2 className="mt-2 text-xl font-black sm:text-2xl">{step?.title}</h2><p className="mt-3 leading-7 text-slate-600">{step?.instruction}</p>{step&&<div className="mt-6"><Answer step={step} value={answers[step.id]} onChange={value=>{setAnswers(a=>({...a,[step.id]:value}));setFeedback(null)}}/></div>}{feedback&&<div className={`mt-5 rounded-xl p-4 text-sm font-semibold ${feedback==="correct"?"bg-emerald-50 text-emerald-800":"bg-amber-50 text-amber-900"}`}>{feedback==="correct"?t.correct:t.wrong}</div>}<div className="mt-7 flex justify-end">{feedback==="correct"?<button onClick={advance} className="rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white">{index===lab.content.steps.length-1?t.finish:t.next}</button>:<button disabled={!answered||busy} onClick={check} className="rounded-xl bg-indigo-700 px-5 py-3 font-bold text-white disabled:opacity-40">{busy?<Loader2 className="animate-spin"/>:feedback==="wrong"?t.retry:t.check}</button>}</div></section></div></article>;
}
