...
// Base assoluta per canonical/og
...
export async function generateMetadata(
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE}${PATH_BY_LANG[lang]}`,
      languages,
    },
    openGraph: {
      type: "article",
      title: `${getLabel(T.title, lang)} | CertifyQuiz`,
      description: {
        it: "Informazioni sul trattamento dei dati personali su CertifyQuiz.",
        en: "Information about personal data processing on CertifyQuiz.",
        fr: "Informations sur le traitement des données personnelles sur CertifyQuiz.",
        es: "Información sobre el tratamiento de datos personales en CertifyQuiz.",
      }[lang],
      url: `${SITE}${PATH_BY_LANG[lang]}`,
    ...
    <main className="mx-auto max-w-3xl px-6 py-10 text-slate-800">
      ...
      <Section title={getLabel(L.dataTypes, lang)}>
        <ul className="list-disc ml-5 space-y-1">
          {T.dataTypesTxt[lang].map((line, i) => (
            <li key={i}>{line}</li>
          ))}
        </ul>
      </Section>
      ...
      <Section title={getLabel(L.purposes, lang)}>
        <ul className="list-disc ml-5 space-y-1">
          {T.purposesList[lang].map((line, i) => (
            <li key={i}>{line}</li>
          ))}
        </ul>
      </Section>
